...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        void ApplySafeArea (Rect safeArea)
        {
            ...
            var oldAnchorMin = Panel.anchorMin;
            var oldAnchorMax = Panel.anchorMax;
            var newAnchorMin = Vector2.zero;
            var newAnchorMax = Vector2.zero;

            // Check for invalid screen startup state on some Samsung devices (see below)
            if (Screen.width > 0 && Screen.height > 0)
            {
                // Convert safe area rectangle from absolute pixels to normalised anchor coordinates
                newAnchorMin = safeArea.position;
                newAnchorMax = safeArea.position + safeArea.size;
                ...
                if (newAnchorMin.x >= 0 && newAnchorMin.y >= 0 && newAnchorMax.x >= 0 && newAnchorMax.y >= 0)
                {
                    Panel.anchorMin = newAnchorMin;
                    Panel.anchorMax = newAnchorMax;
                }
            }

            if (Logging)
            {
                Debug.LogFormat ("New safe area applied to {0}: x={1}, y={2}, w={3}, h={4} on full extents w={5}, h={6}, anchorMin={7}=>{8}, anchorMax={9}=>{10}",
                name, safeArea.x, safeArea.y, safeArea.width, safeArea.height, Screen.width, Screen.height, oldAnchorMin, newAnchorMin, oldAnchorMax, newAnchorMax);
            }
        }

        public int GetTopExtendArea()
        {
            var res = CalculateTopBottom(Panel);

            //Debug.Log($"GetTopExtendArea anchorMin: {Panel.anchorMin}, anchorMax: {Panel.anchorMax}, top: {res.top}, bottom: {res.bottom}");

            return res.top;
        }

        public int GetBottomExtendArea()
        {
            var res = CalculateTopBottom(Panel);
        ...
        public (int top, int bottom) CalculateTopBottom(RectTransform panel)
        {
            var parentHeight = panel.parent.GetComponent<RectTransform>().rect.height;
            var max = parentHeight * panel.anchorMax.y;
            var min = parentHeight * (1 - panel.anchorMin.y);
            var top = parentHeight - max;
            var bottom = parentHeight - min;
